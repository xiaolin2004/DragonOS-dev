...
use crate::libs::{mutex::Mutex, wait_queue::WaitQueue};

use super::bio::{BioEnqueueError, BioType, SubmittedBio};
...
impl BioRequest {
    ...
    pub fn can_merge(&self, rq_bio: &SubmittedBio) -> bool {
        ...
        rq_bio.sid_range().lba_start == self.sid_range.lba_end
            || rq_bio.sid_range().lba_end == self.sid_range.lba_start
    }
    ...
    pub fn merge_bio(&mut self, rq_bio: SubmittedBio) {
        ...
        if rq_bio.sid_range().lba_start == self.sid_range.lba_end {
            self.sid_range.lba_end = rq_bio.sid_range().lba_end;
            self.bios.push_back(rq_bio);
        } else {
            self.sid_range.lba_start = rq_bio.sid_range().lba_start;
